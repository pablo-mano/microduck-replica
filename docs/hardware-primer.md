...
> [Hardware Design Reverse Engineering](硬件方案逆向.md) is the **derivation process and evidence sources** (for those who need to verify),
> [Hardware Specifications Quick Reference](硬件规格速查.md) is a **one-page spec sheet** (for those who already understand).
...
**Which config to buy**: 2G / 16G eMMC. Derivation process in [Electronics Sourcing List](电控采购清单.md#which-config-to-buy).
...
See [Voltage Truth](执行器选型.md#voltage-truth-xl330-runs-overvoltage) for details.
...
2. **Use off-the-shelf modules instead** — A ¥22 ST/SC half-duplex adapter board + a UBEC replaces the two essential functions "servo bus" and
   "power distribution", ToF connects directly to i2c3. Cost is loss of onboard audio. See [Electronics Sourcing List](电控采购清单.md#six-two-pcbs).
...
- **Protocol and register layout fully reverse-engineered**, implement according to spec ([Hardware Design Reverse Engineering](硬件方案逆向.md))
...
| 12-byte register layout | `robotd` source code, see [Hardware Design Reverse Engineering](硬件方案逆向.md) |
